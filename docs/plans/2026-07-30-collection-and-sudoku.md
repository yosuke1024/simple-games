...
4. **i18n**: 言語ごとに1ファイルの単一カタログ(型で全キー強制)を維持。既存キーの一括改名はせず、
...
| M6 | 2048(盤ロジックは小さい。undo・統計・デイリーは共通パターン踏襲) | ✅ 2026-07-30(レベルは作らない。Undo はタイル出現列も巻き戻す) |
| M7 | Sliding Puzzle(画像なし数字タイル・可解性保証シャッフル) | ✅ 2026-07-30(Hint なし。合法手だけで崩すので可解性が自明) |
| M8 | Minesweeper(初手安全保証・旗/開閉 UI) | ✅ 2026-07-30(推測なし保証。Undo なし・Hint あり) |
| M-L10N | 14言語への拡張(§7)+ ストアローカライズ構造 | ✅ 2026-07-30(ar は保留。ストア文言の言語別分割は未実施) |
| M9 | リリース仕上げ: アイコン、store listing 最終化、IAP 本番接続、Android ビルド | アイコン ✅ / チェックリスト ✅ / IAP 接続は人間作業 |
...
3. ~~2048 以降の各ゲーム仕様書~~ → `GAME_2048_RULES.md` / `SLIDING_PUZZLE_RULES.md` /
   `MINESWEEPER_RULES.md` を執筆済み。次のゲームでも同じ順序(先にルール、次に実装)。
4. iOS 展開の時期(現状 Android のみ。IAP 文言はストア中立にしておく)。
5. ~~Sudoku のアイコン/スプラッシュ用モチーフ~~ → コレクションのアイコンを
   ゲーム非依存の4タイルに刷新済み(M9)。ゲーム別アイコンは作らない
   (ランチャーに出るのはコレクション1つだけであり、ゲームの識別は
   ホーム画面のグリフとアクセント色が担う)。
6. **`rng.ts` の共通化**: 5ゲームすべてが同じ 20 行をコピーしている。
   `packages/` への抽出を検討する条件は満たしたので、次の変更で判断する
   (ゲームの Pure TS 純度を保ったまま抽出できるかが論点)。
   → **2026-09-05 決着: 共通化しない。** 30 本の実装は 19 種類に分かれていて
   統一すれば乱数列(=プレイヤーの盤面)が変わりうる。ARCHITECTURE.md
   「将来の共通化」を参照。
...
- **地域バリアントの親言語フォールバック**は `i18n/index.ts` の `matchLocale` に実装済み
  ...
  中国語対応時に同関数へ追加する。
- **実績(2026-07-30)**: **14言語・215キー**へ拡張済み。以下は拡張前の記述。
- **拡張前**: 5言語(en/ja/hi/th/id)・約100キー。Messages 型で全キーの存在をコンパイル時に
