...
| **B** | `simple-games-club`  | **初回コミット済(2026-09-09、API v1 全部 + 契約テスト + Dockerfile + railway.toml)。残: template・Web ビルド同梱・事業者上の往復。** Node + SQLite + 1 volume。club.md §5 の API v1 を満たす。§5 の JSON を fixture にした契約テスト。Web ビルドを `/` と `/join` で配る(`/join/` → `/join`)。CORS(自 origin / `https://localhost` / `capacitor://localhost`)。token は hash 保存。`X-Club-Api: 1`。事業者の template(volume / healthcheck / `CLUB_SETUP_KEY` / serverless 既定 OFF)。Dockerfile 1 枚で他の事業者でも動く | 契約テスト緑。手元(localhost)と事業者上の 1 台で `/health` → `/claim` → `/join` → `/challenges` の往復を確認                        |
