...
#### Phase 1 の実施結果(2026-08-08)

3 色を**同時に**選定した — 3 本は同じリリースで収録され、ホームのロジック節で
隣り合うタイルになるため、互いの距離を測らずに 1 本ずつ決めると後続の選択肢を先に
潰す(BRAND.md の Spider Solitaire / FreeCell と同じ理由)。ただし
**`titleAccents` と `styles.css` への登録は各ゲームの実装 PR で行う** —
「存在しないゲームのアクセントを先回りして定義しない」(BRAND.md)ため、
PR1 に入るのは Takuzu の 1 エントリだけで、下 2 行は**予約**である。

手順どおりの実測: CIE76 で既存 22 色の最接近ペアを数え直したところ BRAND.md 記載の
床がそのまま再現した(ライト `#2f6f62`×`#18787b` と `#6d5192`×`#5d5aa8` が 10.9、
ダーク `#a893cf`×`#9d9be0` が 7.9)。彩度・明度の実測レンジも記載どおり
(ライト S21–67 / L27–51、ダーク S24–59 / L57–74)。1° 刻みで色相を一周させても
**どの帯にも 30° 以内に既存色がいた** — Checkers の所見は 22 タイトルでも変わらない。

床は満たすが**最大化はしない**: ΔE を最大化すると原色 3 本(赤・紫・桃)が出て、
数値は通るがシリーズの静けさを壊す。彩度・明度をパレット中央値(ライト S44/L37、
ダーク S43/L65)に寄せる目的関数に切り替え、床は 1.15 倍の余裕を要求した
(手順 5 の再測定用の頭金)。ホームの隣接は logic 節の並び
(Sudoku / Minesweeper ・ Nonogram / Kakuro ・ Futoshiki / Takuzu)から
既存 3 本に対してライト 15 / ダーク 11 の追加床を課した。`--warn`(`#c2603f`)も
距離の対象に含めた — 「操作を間違えた」の色と紛れるアクセントは作れない。

| ゲーム    | 色           | ライト    | ダーク    | 最接近(ライト / ダーク)        | 白文字 / 紙 |
| --------- | ------------ | --------- | --------- | ------------------------------ | ----------- |
| Takuzu    | ワイン       | `#88355e` | `#cb7ea4` | Block Puzzle 14.6 / Gomoku 9.8 | 7.73 / 6.79 |
| Kakuro    | タバコ(予約) | `#794e2f` | `#cb9e7e` | Sliding Puzzle 12.6 / 9.2      | 7.13 / 6.27 |
| Futoshiki | パイン(予約) | `#29603a` | `#74c88d` | Number Recall 12.6 / 9.4       | 7.42 / 6.52 |

3 色の相互距離はライト 40.8〜71.4・ダーク 38.8〜80.0 で、床から大きく離れている。
コントラストは 3 色とも白文字 7.1 以上・**紙の上でも 6.2 以上**で、ドリル 3 本が
詰めた 4.58〜5.76 より余裕がある(§コントラストの「先行 4 色は紙で 4.5 未満」の
列には入らない)。soft / onDark の値は既存 22 エントリからの回帰
(softLight = accentS×0.97 / L90%、softDark = darkS×0.71 / L18%、onDark = L8%)で
出しており、目分量ではない。

**予約 2 色は PR2 / PR3 で必ず測り直す**(手順 5)。本計画の実行中に別ブランチが
アクセントを足したら、この表の値は無効になる。
