...
| グリフ          | FreeCell `♣` / Spider Solitaire `♥`(Solitaire の `♠` と合わせ、カード3本がスートを着る)                                                                                                                                                                                                  |
| アクセント色    | FreeCell = **スチールシアン系**(Water Sort `#33708c` と Minesweeper `#4a5a72` の間の空き、要距離確認) / Spider = **ワイン・バーガンディ系**(Memory Match のローズ `#9e5468` より暗く、`--warn` のテラコッタと要距離確認)。BRAND.md の基準(本文 4.5:1 / アクセント面 3:1、両テーマ)で確定 |
