...
Connect Four, Brick Breaker, Nonogram, Number Match, Quick Math, Schulte Table,
Number Recall, Water Sort, Sliding Puzzle, Memory Match, Sky Fighter,
Bunny Hop" の 20 本)。
...
| FreeCell         | くすんだ菫               | `#853795` | `#c288d0` | コレクションホームで Solitaire の真下に来る唯一のタイトルなので、三色目のカード台の緑ではなくフェルトから最も遠い色相を採る。Nonogram のプラムと Block Puzzle のオーキッドの間に残っていた最大の空き(どちらからも ΔE 23)                                                                                                                                                                          |
...
1. 候補を機械的に絞る: 白文字 4.5:1 以上・紙に対して 3:1 以上・シリーズの彩度と明度の
   範囲内(彩度 20〜50%、明度 28〜48%)。
2. 既存全色との**知覚距離(CIELAB ΔE)**を測る。色相角だけでは彩度と明度の差を
   見落とす。**基準は既存で最も近いペア** — ライト `#6d5192` × `#5d5aa8`(ΔE 10.9)、
   ダーク `#a893cf` × `#9d9be0`(ΔE 7.9)。ここより近い色は、コレクションで最も
   紛らわしい 2 枚を作ることになるので採らない。
