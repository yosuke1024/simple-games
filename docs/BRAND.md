...
- **アプリの画面に出る文言**: `apps/simple-games/src/i18n/locales/*.ts`。
  ...
  **約束を運ぶキーは `tagline` だけではない**(2026-08-30 追加)。Web 版の
  ...
  [WEB_VERSION.md](WEB_VERSION.md)「アプリへの送客」)が、Web 版のコレクション
  ホームだけに出る 2 つ目の promise-bearing なキー対である。オフラインの言い方を
  ...
  **2026-09-11(issue #192)にこのカードは常設になった。** 文面は変えていないが、
  読まれる回数は「そのブラウザで 1 度」から「訪問のたび」に変わっている ——
  緊急性・広告・価格の禁止(下の「使用禁止の表現」)は、毎回出るものだからこそ
  前より重く効く。
