...
ストア文面では収録済みのゲーム名を具体的に挙げる(現在は "Sudoku, Minesweeper,
2048, Number Match, Sliding Puzzle" の 5 本)。未収録のゲーム名は、**実際に収録された
時点で**初めて足す。ゲーム名は検索流入の中核だが、不自然なキーワード詰め込みはしない。
...
| 2048 | 琥珀 | `#a86a17` | `#dda54a` | タイルの色階調が数字の上昇につれ暖色へ寄るので、クロムはそれに逆らわず合わせる。隣の陶土色とはコレクション一覧で見分けがつく距離 |
...
- 新タイトルを追加するときは `titleAccents` に1エントリ足すだけにする。
  **存在しないゲームのアクセントを先回りして定義しない。** 現在エントリを持つのは
  収録済みの 5 タイトルだけで、Nonogram 以降の色は未定義。
