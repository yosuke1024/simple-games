...
Sliding Puzzle, Memory Match, Sky Fighter, Bunny Hop" の 18 本)。
...
| Number Recall  | エメラルド               | `#2f7a3e` | `#76c37f` | 伏せ札は色ではなく面で区別する。アクセントは次の数字と、外したときの印だけ。Solitaire と 2048 の間に残っていた最後の広い色相                          |
...
- 新タイトルを追加するときは `titleAccents` に1エントリ足すだけにする。
  **存在しないゲームのアクセントを先回りして定義しない。** 現在エントリを持つのは
  収録済みの 18 タイトルだけ。
- **色相環はここでほぼ埋まった。** 18 タイトルで隣り合う色相の間隔は最も狭いところで
  13〜15°(Brick Breaker / Quick Math / Bunny Hop、Sudoku / Schulte Table / Water Sort)に
  なっており、**次のタイトルは色相ではなく明度と彩度で差を作ることになる**。
  Minesweeper のスレートと Number Match の藍が 3° 差で成立しているのが先例である。
...
ため、3 本とも紙の上で 4.5:1 を満たす値に詰めてある(4.58 / 4.60 / 4.64)。
