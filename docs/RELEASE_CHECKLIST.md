...
実際に開くかは実機でしか分からない** —— とくに iOS は `capacitor://localhost` という
custom scheme 上で動き、`navigator.share` / `navigator.clipboard` が生きているかは
OS 版に依存する。plugin を入れる / 入れないの判断はここの結果で決める。

- [ ] Android 実機で「共有」を押すと OS の共有シートが開く
- [ ] iOS 実機で同じこと。開かない場合は**リンクがコピーされ、「リンクをコピー
      しました」が出る**こと(どちらも起きないなら公式 Share plugin の追加を検討する)
...
- [ ] Android / iOS の**アプリ**で画像付きの共有シートが開くか、開かなければ
      文面だけの共有シートに落ちること(どちらでもゲームは止まらない)。画像が
      付かない場合は公式 Share plugin + Filesystem の追加を検討する
