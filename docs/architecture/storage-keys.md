...
- `sg.adState` / `sg.rcCache` は廃止(インタースティシャル頻度制御と
  Remote Config キャッシュは存在しない)。`sg.webAppPrompt` も廃止した
  (2026-09-11、issue #192 — Web 版のアプリ案内カードが常設になり、
  「何局遊んだか / 表示済みか」を覚える必要が無くなった)。**既存ブラウザに
  残っている値は読まないし書かないし消さない。** 表示可否に影響しないので、
  削除のためだけの migration は作らない。
...
- **バックアップ層はレコードの中身を解釈しない。** 復元は各レコードの `SchemaDef`
  を通すので、古いバックアップはそのゲーム自身の migration を通って入る。
  生の文字列を読み書きする `loadRaw` / `saveRaw` も他の操作と同じ直列化キューに
  載るため、飛んでいた保存が復元の後から着地することはない。
