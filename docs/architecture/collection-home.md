...
- **グリッドはカテゴリ別のセクションに分ける**(`registry.ts` の `GAME_CATEGORIES`、
  ...
  全ゲームを 1 枚のグリッドに敷くと結局全タイトルを読んで探すことになるため、
...
- **タイルはタイトルごとのアクセント色**を着る。名前を読まなくても色と位置で
  ...
  担う」に合わせるための実装。
- **Web 版のアプリ案内カード**(`ui/components/WebAppStoreCard.tsx`)は
  **ヒーローの直後・「お気に入り」より上**に置く(2026-09-11、issue #192)。
  ネイティブでは何も描かない(オフラインの Web でも同じ)ので、アプリ側の
  並びは以前のままである。**棚より上なのは、棚が使うほど伸びるから** ——
  下に置くと、このカードが向いている「初めて来た人」ほど深く埋まる。
  内容・禁止事項・常設である理由は
  [WEB_VERSION.md](../WEB_VERSION.md)「アプリへの送客」が正本。ここで守るのは
  **位置と大きさ**だけ:主役はゲームなので、Hero 化させず、mobile の 1 画面目から
  グリッドが見えている高さに収める。
...
- **「お気に入り」**(`app/favoriteGames.ts` / `sg.favorites`)を「最近遊んだ」の
  **上**に置く(issue #109)。表示順は(Web のカードを除けば)
  お気に入り → 最近遊んだ → カテゴリ。
