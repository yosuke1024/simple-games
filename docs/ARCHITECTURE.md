...
- **全ゲームは 2 列グリッド**(`.game-grid`)。1 行 1 ゲームの縦リストは 8 本で
  ほぼ 1 画面、16 本で 2 画面を超え、末尾のタイトルが常にスクロールの先になる。
...
- 「ローカルデータ削除」は `sg.*` と全ゲームのキーを消す。**書き込みは全レイヤーで
  fire-and-forget**(ゲームを保存待ちで止めない)なので、削除の瞬間に飛んでいる
  書き込みがありうる。`repo.ts` は削除回数を数えていて、削除に追い越された書き込みは
  着地後に自分を消す。これがないと、消したはずのレコードが復活して削除ボタンが嘘をつく
  — ゲームは 1 手ごとに保存するので、宙に浮いている可能性が最も高いのは盤面である。
