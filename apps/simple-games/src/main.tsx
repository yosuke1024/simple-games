...
/**
 * Boot: local-only reads (fast, offline-first), each guarded on its own in
 * `app/boot.ts`. The shell loads only the shared records; each game loads its
 * own when opened. Network-dependent work (the banner SDK) starts AFTER first
 * render and is never awaited — the app never waits for the network
...
async function boot(): Promise<void> {
  ...
  const settings = await initShellState();
  ...
  // gate the app. With the ad removal in effect the ad SDK is never
