import type { Messages } from './en';

export const ptBR: Messages = {
  tagline: 'Funciona offline. Sem conta. Sem paywall.',

  resume: 'Continuar',
  dailyChallenge: 'Desafio diário',
  dailyDoneBadge: 'Concluído hoje',
  howToPlay: 'Como jogar',
  statistics: 'Estatísticas',
  settings: 'Configurações',

  modeLevel: 'Nível {n}',
  levelSelect: 'Escolher nível',
  levelLocked: 'Nível {n}, bloqueado',
  nextLevel: 'Próximo nível',
  levelsTitle: 'Níveis',
  reachedLevel: 'Nível alcançado',

  score: 'Pontos',

  dailyPast: 'Diários anteriores',
  dailyToday: 'Hoje',
  dailyBacklogHint: 'Conclua um dia para desbloquear o anterior.',
  modeDaily: 'Diário',
  freePlay: 'Jogo livre',
  freePlayNote: 'Um tabuleiro novo, quando você quiser.',
  difficulty: 'Dificuldade',
  undo: 'Desfazer',
  hint: 'Dica',
  timeLabel: 'Tempo',
  movesLabel: 'Jogadas',

  tryAgain: 'Repetir o tabuleiro',
  newGame: 'Novo jogo',
  backHome: 'Início',

  confirmNewGameTitle: 'Começar um novo jogo?',
  confirmNewGameBody: 'Seu jogo atual será perdido.',
  cancel: 'Cancelar',
  confirm: 'Começar',

  startPlaying: 'Jogar',
  next: 'Avançar',
  back: 'Voltar',
  close: 'Fechar',

  language: 'Idioma',
  languageSystem: 'Sistema',
  theme: 'Tema',
  themeSystem: 'Sistema',
  themeLight: 'Claro',
  themeDark: 'Escuro',
  sound: 'Som',
  vibration: 'Vibração',
  reducedMotion: 'Reduzir movimento',
  privacyPolicy: 'Política de Privacidade',
  termsOfUse: 'Termos de uso',
  adPrivacyOptions: 'Opções de privacidade de anúncios',
  // Backup & Restore (issue #160). One file the player moves themselves —
  // Simple Games has no account, no cloud save, and nothing that uploads it.
  backupTitle: 'Backup e restauração',
  backupBody: 'Leve seu progresso para outro aparelho ou guarde uma cópia.',
  backupPrivacyNote:
    'O backup é um arquivo sem criptografia no seu aparelho. O Simple Games nunca faz upload dele, e só você decide onde guardá-lo.',
  backupPurchaseNote:
    'A compra para remover os anúncios não faz parte do backup. Restaure-a pela loja.',
  backupExport: 'Exportar backup',
  backupRestore: 'Restaurar backup',
  backupExportFailed: 'Não foi possível criar o arquivo de backup.',
  backupRestoreConfirmTitle: 'Substituir os dados deste aparelho?',
  backupRestoreConfirmBody:
    'Tudo o que está salvo aqui — jogos em andamento, estatísticas e configurações — é substituído pelo backup de {date}. Não dá para desfazer.',
  backupRestoreDone: 'Seus dados foram restaurados.',
  backupRestoreFailed: 'Não foi possível concluir a restauração. Seus dados atuais foram mantidos.',
  backupFileUnreadable: 'Esse arquivo não é um backup do Simple Games.',
  backupFileDamaged: 'Este arquivo de backup está danificado. Nada foi alterado.',
  backupFileNewer:
    'Este backup foi feito por uma versão mais nova do Simple Games. Atualize o aplicativo e tente de novo.',
  resetData: 'Apagar dados locais',
  resetConfirmTitle: 'Apagar todos os dados locais?',
  resetConfirmBody:
    'Isso remove seu jogo, suas estatísticas e suas configurações deste dispositivo. Não é possível desfazer.',
  delete: 'Apagar',
  version: 'Versão',

  played: 'Partidas jogadas',
  cleared: 'Partidas concluídas',
  totalTime: 'Tempo total de jogo',
  bestTime: 'Mais rápido',

  // Collection shell
  gamesHeading: 'Jogos',
  recentHeading: 'Jogados recentemente',
  favoritesHeading: 'Favoritos',
  addToFavorites: 'Adicionar aos favoritos',
  removeFromFavorites: 'Remover dos favoritos',
  addToHomeScreen: 'Adicionar à tela inicial',
  searchGames: 'Buscar jogos',
  searchNoResults: 'Nenhum jogo corresponde.',
  categoryLogic: 'Lógica',
  categoryCards: 'Cartas',
  categoryPuzzle: 'Quebra-cabeça',
  categoryBoard: 'Jogos de tabuleiro',
  categoryArcade: 'Arcade',
  categoryDrills: 'Exercícios',
  backToGames: 'Todos os jogos',
  gameLoading: 'Carregando…',
  gameLoadFailed: 'Não foi possível carregar o jogo.',
  gameLoadRetry: 'Tentar novamente',
  learnMore: 'Saiba mais',

  // About & open source
  aboutTitle: 'Sobre',
  viewSource: 'Ver código-fonte',
  reportBug: 'Relatar um erro',
  suggestGame: 'Sugerir um jogo',
  viewLicenses: 'Ver licenças',

  // Ads & support
  removeAdsTitle: 'Remover anúncios e apoiar o Simple Games',
  adSupportBody:
    'O Simple Games se mantém com um pequeno banner de anúncio enquanto você está online. Isso me ajuda a manter e melhorar o app. Prefere sem anúncios? Uma única compra os remove permanentemente.',
  removeAdsAction: 'Remover anúncios',
  restorePurchase: 'Restaurar compra',
  purchaseThanks: 'Os banners de anúncio foram removidos. Obrigado por apoiar o Simple Games.',

  reviewPromptTitle: 'Está gostando do Simple Games?',
  reviewYes: 'Sim, estou gostando',
  reviewNo: 'Nem tanto',
  reviewLater: 'Agora não',
  reviewFeedbackTitle: 'O que pode melhorar?',
  reviewFeedbackBody:
    'Conte para mim por e-mail — leio todas as mensagens. Nada é enviado até você tocar em enviar no app de e-mail.',
  reviewFeedbackAction: 'Escrever e-mail',

  // The browser version's one-time pointer at the app
  webAppPromptTitle: 'Um jeito mais tranquilo de jogar, offline desde o primeiro uso.',
  webAppPromptBody: 'O app abre direto da tela inicial e guarda seu progresso no seu dispositivo.',

  // The optional share on a result screen. {game} is the registry title, a
  // proper noun that is not translated; `cleared` is only used for a real win.
  shareAction: 'Compartilhar',
  shareCleared: 'Completei {game} no Simple Games.',
  sharePlayed: 'Joguei {game} no Simple Games.',
  shareInvite: 'Dá para jogar direto no navegador.',
  shareChallenge: 'Quer tentar também?',
  shareCardCleared: 'Concluído!',
  shareCopied: 'Link copiado',

  // ---- Arcade (Brick Breaker / Sky Fighter) ----
  livesLeft: 'Vidas: {n}',
  levelsCleared: 'Níveis concluídos',
};
