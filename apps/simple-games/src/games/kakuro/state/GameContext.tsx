/**
 * Kakuro's app context: screens, the active session, statistics, progress,
 ...
 * board (docs/KAKURO_RULES.md §9, §11) — so switching modes never costs the
 * player any of them.
 *
...
export interface KakuroContextValue {
  screen: Screen;
  navigate: (screen: Screen) => void;
  session: KakuroSession | null;
  sessions: SavedGames;
  stats: Stats;
  progress: Progress;
  prefs: Prefs;
...
const KakuroContext = createContext<KakuroContextValue | null>(null);

export interface KakuroProviderProps {
  initialStats: Stats;
  initialFlags: Flags;
...
export function KakuroProvider({
  initialStats,
  initialFlags,
  initialProgress,
  initialSessions,
  ...
  const putSession = useCallback((mode: GameMode, next: KakuroSession | null) => {
  ...
  const commitSession = useCallback(
    (next: KakuroSession) => {
      ...
      const previousBestSeconds = previousBestFor(progressRef.current, statsRef.current, next);
      persistStats(
        applySolved(
      ...
      // A free board leaves progress alone (§9), so its record is read
      // against the size's best rather than off the outcome.
  ...
  const activate = useCallback((next: KakuroSession) => {
  ...
  const startFree = useCallback(
    (tier?: FreeTier | null) => {
      beginSession(createFreeSession(tier ?? freeTierRef.current));
    },
    [beginSession],
  );
  ...
  const mutate = useCallback(
    (apply: (s: KakuroSession) => KakuroSession | null): boolean => {
      const current = sessionsRef.current[activeModeRef.current];
  ...
  const place = useCallback(
    (index: number, digit: Digit) => mutate((s) => doPlace(s, index, digit)),
    [mutate],
  );

  const erase = useCallback((index: number) => mutate((s) => doErase(s, index)), [mutate]);

  const toggleNote = useCallback(
    (index: number, digit: Digit) => mutate((s) => doToggleNote(s, index, digit)),
    [mutate],
  );

  const applyUndo = useCallback((): boolean => mutate((s) => doUndo(s)), [mutate]);
  ...
  const value = useMemo<KakuroContextValue>(
    () => ({
      screen,
      navigate,
      session,
  ...
  return <KakuroContext.Provider value={value}>{children}</KakuroContext.Provider>;
}

export function useKakuro(): KakuroContextValue {
  const value = useContext(KakuroContext);
  if (!value) throw new Error('useKakuro must be used inside KakuroProvider');
  return value;
}
