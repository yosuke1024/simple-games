/**
 * Ludo's own strings (issue #38): bundled into the game's chunk, not the
 * entry, and registered on chunk load by ./index.ts. The shell's en.ts stays
 * the source of truth for shared keys, this file for Ludo's.
 *
 * `ludoName` is the literal 'Ludo' in every locale: a game's title is a proper
 * noun and is never translated (docs/I18N_POLICY.md). The three opponents are
 * `CPU 1` / `CPU 2` / `CPU 3` everywhere for the same reason a seat is not a
 * name — it is a direction round the board, and every reader should find the
 * same number in the same chair (docs/LUDO_RULES.md, and Hearts before it).
 *
 * The dice strings say only what is true and measurable: the faces come from
 * the seed, they are uniform, and they do not depend on the seat
 * (docs/LUDO_RULES.md §3). No competitor is named and the word "rigged" — or
 * any denial of it — appears nowhere.
 */
export const en = {
  ludoName: 'Ludo',
...
export type LudoMessages = Record<keyof typeof en, string>;
