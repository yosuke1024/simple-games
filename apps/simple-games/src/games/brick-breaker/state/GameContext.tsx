/**
 * Brick Breaker's app context: screens, the level being attempted,
 * statistics, progress, and persistence. Pure local state — no analytics; the
 * only ad surfaces are the shared BannerSlot and ResultAdSlot the screens
 * render.
 *
 * Unlike the puzzle titles there is no suspended-game record: a real-time
 * board cannot be restored honestly, so leaving a level abandons the attempt
 * and the retry is free (docs/BRICK_BREAKER_RULES.md §10). The simulation
 * itself lives in the game screen's animation loop; this context only hears
 * about attempts starting and ending, and about play seconds to book.
 *
 ...
 * still has its minutes counted (§9).
 */
...
export interface BrickContextValue {
  screen: Screen;
...
const BrickContext = createContext<BrickContextValue | null>(null);

export interface BrickProviderProps {
  initialStats: Stats;
  initialFlags: Flags;
...
export function BrickProvider({
  ...
  // (BrickBoard's `settle`), and both reads start from the ref. Waiting for
  ...
  const value = useMemo<BrickContextValue>(
    () => ({
      screen,
      navigate,
      attempt,
  ...
  return <BrickContext.Provider value={value}>{children}</BrickContext.Provider>;
}

export function useBrickBreaker(): BrickContextValue {
  const value = useContext(BrickContext);
  if (!value) throw new Error('useBrickBreaker must be used inside BrickProvider');
  return value;
}
