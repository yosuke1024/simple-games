...
/**
 * Statistics (docs/BRICK_BREAKER_RULES.md §9): counts and totals. No score
 * (§13) and no streak. Everything here is local — there is no ranking to
 * compare against.
 */
export function BrickStatsScreen() {
  const { goHome, stats, progress } = useBrickBreaker();
  const { t } = useSettings();
