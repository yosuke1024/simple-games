 ...
 */
export function FreeCellDailyScreen() {
  const { goHome, sessions, stats, startDaily } = useFreeCell();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('fcDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="fc-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="fc-daily-badge fc-daily-badge-done">
