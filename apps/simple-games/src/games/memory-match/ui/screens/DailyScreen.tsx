 ...
 * day shows the moves it took — a record of what was played, never a run of
 * days to protect.
 */
export function MemoryDailyScreen() {
  const { goHome, sessions, stats, startDaily } = useMemoryMatch();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('memoryDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="mm-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="mm-daily-badge mm-daily-badge-done">
