 ...
 * (docs/TAKUZU_RULES.md §7, §10).
 *
 * The only best kept is time. There is no move count worth recording — a cell
 * cycles freely under the same tap (§4), so counting taps would measure
 * hesitation, not skill.
 */
import { MAX_LEVEL, type Size, type TakuzuSession } from '../game';
 ...
 */
export function applySolveToProgress(progress: Progress, session: TakuzuSession): ClearOutcome {
  const next: Progress = {
    ...progress,
