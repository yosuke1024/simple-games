 ...
 * the frontier. A solved level wears a quiet check — the record replaying it
 * is measured against lives in its label (§7, §10).
 */
export function TakuzuLevelSelectScreen() {
  const { goHome, sessions, canResume, startLevel, progress } = useTakuzu();
  const { t } = useSettings();
    ...
    cells.push(
      unlocked ? (
        ...
        >
          ...
          {best !== undefined ? (
            <span className="takuzu-level-done" aria-hidden="true">
              ✓
            </span>
          ) : null}
        </button>
