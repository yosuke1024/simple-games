 ...
 * for a *new* free board while one is suspended, which asks first (§7).
 */
export function TakuzuHomeScreen() {
  const {
    navigate,
    sessions,
    progress,
  ...
  return (
    <div className="screen home-screen">
      <GameHomeHeader gameId="takuzu" onBack={exitToCollection} />

      <div className="home-hero">
        {/* The series mark: the two digits the whole game is played with. */}
        <div className="home-logo takuzu-logo" aria-hidden="true">
          01
        </div>
        <h1 className="home-title">{t('takuzuName')}</h1>
      ...
      <div className="home-actions">
        ...
        {/* Free Play (§7「フリープレイ」): a fresh board at the tier below,
        ...
        >
          ...
          <span className="btn-note">
            {freeGame
              ? `${t('resume')} · ${t(`takuzuTier_${freeTierForSize(freeGame.size)}`)}`
              : t(`takuzuTier_${freeTier}`)}
          </span>
        </button>
        {freeGame ? (
