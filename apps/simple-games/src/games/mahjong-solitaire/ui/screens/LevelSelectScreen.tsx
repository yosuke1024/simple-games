 ...
 * the frontier. A cleared level wears a quiet check — the record replaying
 * it is measured against lives in its label (§6, §9).
 */
export function MahjongLevelSelectScreen() {
  const { goHome, sessions, canResume, startLevel, progress } = useMahjong();
  const { t } = useSettings();
    ...
    cells.push(
      unlocked ? (
        ...
        >
          ...
          {best !== undefined ? (
            <span className="mj-level-done" aria-hidden="true">
              ✓
            </span>
          ) : null}
        </button>
