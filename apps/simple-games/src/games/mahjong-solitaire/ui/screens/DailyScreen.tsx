...
/**
 * The daily backlog: today and the weeks behind it, all open (§6). A cleared
 * day shows the time it took — a record of what was played, never a run of
 * days to protect.
 */
export function MahjongDailyScreen() {
  const { goHome, sessions, progress, startDaily } = useMahjong();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('mahjongDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="mj-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="mj-daily-badge mj-daily-badge-done">
