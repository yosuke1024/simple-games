/**
 * The four suit marks: filled SVG shapes drawn in-app so every platform
 * renders the same silhouettes (text suit glyphs render differently per OS
 * webview — the same reason the shell has icons.tsx). Shape tells the suits
 * apart; the red/black colouring is the aid, never the identity.
 *
 * Copied from the shelf's other card games (Solitaire / FreeCell / Gin Rummy)
 * rather than shared, the same way this game's `cards.ts` is: a game stays
 * independent of every other (docs/ARCHITECTURE.md).
 */
