...
/**
 * The daily backlog: today and the weeks behind it, all open (§8). A finished
 * day shows the time it took — a record of what was played, never a run of days
 * to protect.
 */
export function SchulteDailyScreen() {
  const { goHome, progress, startDaily } = useSchulte();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('schulteDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {best !== undefined ? (
                  <span className="schulte-daily-badge">
