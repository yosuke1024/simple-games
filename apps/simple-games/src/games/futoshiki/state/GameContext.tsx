/**
 * Futoshiki's app context: screens, the active session, statistics, progress,
...
const FutoshikiContext = createContext<FutoshikiContextValue | null>(null);

export interface FutoshikiProviderProps {
  initialStats: Stats;
  initialFlags: Flags;
...
export function FutoshikiProvider({
  initialStats,
  initialFlags,
  initialProgress,
  initialSessions,
  ...
  const activate = useCallback((next: FutoshikiSession) => {
  ...
  const mutate = useCallback(
    (apply: (s: FutoshikiSession) => FutoshikiSession | null): boolean => {
      const current = sessionsRef.current[activeModeRef.current];
  ...
  return <FutoshikiContext.Provider value={value}>{children}</FutoshikiContext.Provider>;
}

export function useFutoshiki(): FutoshikiContextValue {
  const value = useContext(FutoshikiContext);
  if (!value) throw new Error('useFutoshiki must be used inside FutoshikiProvider');
  return value;
}
