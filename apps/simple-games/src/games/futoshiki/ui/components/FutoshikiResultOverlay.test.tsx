 ...
 * (docs/FUTOSHIKI_RULES.md §10) — no score, no streak, and a personal best
...
import { createLevelSession, doPlace, EMPTY, type Digit, type FutoshikiSession } from '../../game';
import type { LastResult } from '../../state/GameContext';
import { FutoshikiResultOverlay } from './FutoshikiResultOverlay';

/** Plays a level to completion so the overlay has a real solved session. */
function solvedSession(level: number, elapsedSeconds: number): FutoshikiSession {
  let session = createLevelSession(level);
  for (let index = 0; index < session.solution.length; index++) {
    if (session.board.givens[index] !== EMPTY) continue;
    const next = doPlace(session, index, session.solution[index] as Digit);
    if (next) session = next;
  }
  return { ...session, elapsedSeconds, mistakeCount: 2, hintCount: 1 };
}

function renderOverlay(session: FutoshikiSession, lastResult: LastResult | null) {
  const onRetry = vi.fn();
...
describe('FutoshikiResultOverlay', () => {
