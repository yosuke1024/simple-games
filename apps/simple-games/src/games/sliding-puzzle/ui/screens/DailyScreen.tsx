...
/**
 * The daily backlog: today and the weeks behind it, all open (§7). A solved day
 * shows the moves it took — a record of what was played, never a run of days to
 * protect.
 */
export function SlidingPuzzleDailyScreen() {
  const { goHome, sessions, progress, startDaily } = useSlidingPuzzle();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('slideDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="slide-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="slide-daily-badge slide-daily-badge-done">
