...
/**
 * Statistics per board size (docs/SLIDING_PUZZLE_RULES.md §9): counts, fewest
 * moves, fastest and total time. No score and no streak. Everything here is
 * local — there is no ranking to compare against.
 */
export function SlidingPuzzleStatsScreen() {
  const { goHome, stats, progress } = useSlidingPuzzle();
  const { t } = useSettings();
      ...
      <div className="stats-body">
        <section className="stats-section">
          ...
          <dl className="stats-grid">
            ...
            <div className="stats-row">
              <dt>{t('slideLevelsSolved')}</dt>
              <dd>{solvedLevelCount(progress)}</dd>
            </div>
            <div className="stats-row">
              <dt>{t('slideDailiesSolved')}</dt>
        ...
        {SIZES.map((size: Size) => {
          ...
          return (
            <section key={size} className="stats-section">
              <h2 className="stats-title">{t('slideSizeLabel', { n: size })}</h2>
              <dl className="stats-grid">
                <div className="stats-row">
                  <dt>{t('played')}</dt>
                  <dd>{bucket.played}</dd>
