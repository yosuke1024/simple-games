...
/**
 * Level select: replay any unlocked level (to beat its move count) or continue
 * at the frontier. A solved level shows the fewest moves it has been done in —
 * the record replaying it is measured against (§6).
 */
export function SlidingPuzzleLevelSelectScreen() {
  const { goHome, sessions, canResume, startLevel, progress } = useSlidingPuzzle();
  const { t } = useSettings();
    ...
    cells.push(
      unlocked ? (
        <button
          ...
          aria-label={
            best !== undefined
              ? `${t('modeLevel', { n: level })}, ${t('slideBestMoves')} ${best}`
              : t('modeLevel', { n: level })
          }
          onClick={() => onPick(level)}
        >
          <span className="level-cell-number">{level}</span>
          {best !== undefined ? <span className="slide-level-best">{best}</span> : null}
        </button>
      ) : (
