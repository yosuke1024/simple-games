  ...
  try {
    ...
    const warmup = render(
      <SettingsProvider initialSettings={settingsSchema.defaultValue()}>
        <DinoRunRoot onExit={() => undefined} kv={createMemoryKV(tutorialDone)} />
      </SettingsProvider>,
    );
    ...
    try {
      ...
      expect('__drFrame' in window).toBe(true);
    ...
    expect('__drFrame' in window).toBe(false);
    expect('__drState' in window).toBe(false);
  } finally {
    restoreMedia();
    restoreCanvas();
  }
