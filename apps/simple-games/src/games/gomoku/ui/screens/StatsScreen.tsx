...
/**
 * Statistics (docs/GOMOKU_RULES.md §7): a record per opponent, and total
 ...
 * each strength.
 */
export function GomokuStatsScreen() {
  const { goHome, stats } = useGomoku();
  const { t } = useSettings();
        ...
        {DIFFICULTIES.map((difficulty) => {
          ...
          return (
            <section key={difficulty} className="stats-section">
              <h2 className="stats-title">{t(`gomokuDifficulty_${difficulty}`)}</h2>
