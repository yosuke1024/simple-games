 ...
 * a player who asked to resume their free game into the daily slot: the
 * other game, or a blank screen where the other game isn't.
 */
import { describe, expect, it } from 'vitest';
import { createDailySession, createFreeSession } from '../game';
import { toPersisted } from './gamePersistence';
import { dailyGameSchema, gameSchema } from './schemas';

const SAVED_AT = 1_754_000_000_000;
const freeRecord = toPersisted(createFreeSession(false, 'so-slots'), SAVED_AT);
const dailyRecord = toPersisted(createDailySession('2026-08-07', false), SAVED_AT);

describe('saved-game slots', () => {
  it('loads each record from its own slot', () => {
    expect(gameSchema.validate(freeRecord)?.mode).toBe('free');
