 ...
 * (docs/SOLITAIRE_RULES.md §9, §12): moves and time, each record mentioned
...
import { createFreeSession, type SolitaireSession } from '../../game';
import type { LastResult } from '../../state/GameContext';
import { SolitaireResultOverlay } from './SolitaireResultOverlay';

/** A free deal declared won — the overlay reads the status, not the board. */
function wonSession(moves: number, seconds: number): SolitaireSession {
  return { ...createFreeSession(false), status: 'won', moveCount: moves, elapsedSeconds: seconds };
}

function renderOverlay(session: SolitaireSession, lastResult: LastResult | null) {
...
describe('SolitaireResultOverlay', () => {
  it('shows nothing while the deal is still in play', () => {
    renderOverlay(createFreeSession(false), null);
