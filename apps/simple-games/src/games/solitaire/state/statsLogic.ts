 ...
 * (docs/SOLITAIRE_RULES.md §6, §9).
 *
 * The win rate is honest arithmetic over unscreened deals (§5): played
 * counts every deal begun, won counts the wins, and nothing massages the
 * ratio.
 */
 ...
 */
export function applyWon(stats: Stats, session: SolitaireSession): WinOutcome {
  const next = clone(stats);
