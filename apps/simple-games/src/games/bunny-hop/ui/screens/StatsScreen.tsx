...
/**
 * Statistics (docs/BUNNY_HOP_RULES.md §9): counts, one best score, and totals.
 * No streak. Everything here is local — there is no ranking to compare
 * against, and no leaderboard to put a name into.
 */
export function BunnyStatsScreen() {
  const { goHome, stats } = useBunnyHop();
  const { t } = useSettings();
