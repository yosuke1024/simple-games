/**
 * Bunny Hop's app context: screens, the run being attempted, statistics, and
 ...
 * (docs/BUNNY_HOP_RULES.md §10). The simulation itself lives in the board's
...
export interface BunnyContextValue {
  screen: Screen;
...
const BunnyContext = createContext<BunnyContextValue | null>(null);

export interface BunnyProviderProps {
  initialStats: Stats;
  initialFlags: Flags;
...
export function BunnyProvider({
  ...
  // (BunnyBoard's `settle`), and both reads start from the ref. Waiting for
  ...
  const value = useMemo<BunnyContextValue>(
    () => ({
      screen,
      navigate,
      attempt,
  ...
  return <BunnyContext.Provider value={value}>{children}</BunnyContext.Provider>;
}

export function useBunnyHop(): BunnyContextValue {
  const value = useContext(BunnyContext);
  if (!value) throw new Error('useBunnyHop must be used inside BunnyProvider');
  return value;
}
