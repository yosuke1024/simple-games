  ...
  try {
    ...
    const warmup = render(
      <SettingsProvider initialSettings={settingsSchema.defaultValue()}>
        <SkyFighterRoot onExit={() => undefined} kv={createMemoryKV(tutorialDone)} />
      </SettingsProvider>,
    );
    ...
    try {
      ...
      expect('__sfFrame' in window).toBe(true);
    ...
    expect('__sfFrame' in window).toBe(false);
    expect('__sfState' in window).toBe(false);
  } finally {
    restoreMedia();
    restoreCanvas();
  }
