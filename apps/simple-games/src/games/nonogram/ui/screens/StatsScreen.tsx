...
/**
 * Statistics per board size (docs/NONOGRAM_RULES.md §9): counts, fastest and
 * total time. No score and no streak. Everything here is local — there is no
 * ranking to compare against.
 */
export function NonoStatsScreen() {
  const { goHome, stats, progress } = useNonogram();
  const { t } = useSettings();
      ...
      <div className="stats-body">
        <section className="stats-section">
          ...
          <dl className="stats-grid">
            ...
            <div className="stats-row">
              <dt>{t('nonoLevelsSolved')}</dt>
              <dd>{solvedLevelCount(progress)}</dd>
            </div>
            <div className="stats-row">
              <dt>{t('nonoDailiesSolved')}</dt>
        ...
        {SIZES.map((size: Size) => {
          ...
          return (
            <section key={size} className="stats-section">
              <h2 className="stats-title">{t('nonoSizeLabel', { n: size })}</h2>
              <dl className="stats-grid">
                <div className="stats-row">
                  <dt>{t('played')}</dt>
                  <dd>{bucket.played}</dd>
