...
/**
 * The daily backlog: today and the weeks behind it, all open (§6). A solved
 * day shows the time it took — a record of what was played, never a run of
 * days to protect.
 */
export function NonoDailyScreen() {
  const { goHome, sessions, progress, startDaily } = useNonogram();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('nonoDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="nono-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="nono-daily-badge nono-daily-badge-done">
