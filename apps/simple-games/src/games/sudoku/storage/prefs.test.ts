/**
 * The Free Play tier joined the preferences after release (§9): a record
 * without it is an older one, not a broken one.
 */
import { describe, expect, it } from 'vitest';
import { prefsSchema } from './schemas';

describe('prefs', () => {
  it('starts the picker at medium', () => {
    expect(prefsSchema.defaultValue().freeDifficulty).toBe('medium');
  });

  it('reads a record from before the picker existed', () => {
    expect(prefsSchema.validate({ schemaVersion: 1, highlightMistakes: false })).toEqual({
      schemaVersion: 1,
      highlightMistakes: false,
      freeDifficulty: 'medium',
    });
  });

  it('keeps the tier that was chosen, and rejects one that is not a tier', () => {
    expect(
      prefsSchema.validate({ schemaVersion: 1, highlightMistakes: true, freeDifficulty: 'hard' })
        ?.freeDifficulty,
    ).toBe('hard');
    expect(
      prefsSchema.validate({ schemaVersion: 1, highlightMistakes: true, freeDifficulty: 'x' }),
    ).toBeNull();
  });
});
