...
/**
 * Statistics (docs/CONNECT_FOUR_RULES.md §7): a record per opponent, and
 ...
 */
export function ConnectFourStatsScreen() {
  const { goHome, stats } = useConnectFour();
  const { t } = useSettings();
        ...
        {DIFFICULTIES.map((difficulty) => {
          ...
          return (
            <section key={difficulty} className="stats-section">
              <h2 className="stats-title">{t(`fourDifficulty_${difficulty}`)}</h2>
