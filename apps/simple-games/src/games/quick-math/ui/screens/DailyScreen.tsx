...
/**
 * The daily backlog: today and the weeks behind it, all open (§7). A finished
 * day shows the time it took — a record of what was played, never a run of days
 * to protect.
 */
export function QuickMathDailyScreen() {
  const { goHome, sessions, progress, startDaily } = useQuickMath();
  ...
  return (
    <div className="screen daily-screen">
      ...
      <p className="daily-hint">{t('qmathDailyBacklogHint')}</p>
          ...
          return (
            ...
            >
              ...
              <span className="daily-row-meta">
                {date === inProgress ? (
                  <span className="qmath-daily-badge">{t('resume')}</span>
                ) : best !== undefined ? (
                  <span className="qmath-daily-badge">
