...
function search(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const value = goesOn
      ? search(state, toMove, me, depth, level + 1, moveTo(move), alpha, beta)
      : search(state, opponentOf(toMove), me, depth - 1, level + 1, NO_CAPTURE, alpha, beta);
    unmake(state, move);
...
function bestAtDepth(state: Search, me: Side, turns: readonly Turn[], depth: number): Turn | null {
  ...
  for (const turn of turns) {
    ...
    let value: number;
    try {
      value = search(state, opponentOf(me), me, depth - 1, 1, NO_CAPTURE, -Infinity, Infinity);
    } catch (error) {
...
export function chooseCpuTurn(input: CpuTurnInput): Turn | null {
  const { board, side, difficulty, seed, moveCount, continueFrom = null } = input;
  ...
  if (difficulty === 'easy' || rootTurns.length === 1) return rootTurns[0]!;

  const state = createSearch(board);

  if (difficulty === 'normal') {
    const best = bestAtDepth(state, side, rootTurns, NORMAL_DEPTH);
  ...
  for (let depth = 1; depth <= HARD_MAX_DEPTH; depth++) {
    const best = bestAtDepth(state, side, rootTurns, depth);
    if (best === null) break;
    chosen = best;
  }
  return chosen;
