...
ゲームの名前、結果画面が表示した事実(時間・スコアなど、本人がその画面で見ている
数字だけ)、リンク、そして端末が対応していれば画像カードを送ります。勝敗が確定
した結果だけが「クリアしました」を名乗り、敗北・引き分け・エンドレスの終了は
「遊びました」になります。共有しても報酬・機能解放・再催促はなく、共有した
